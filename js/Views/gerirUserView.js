...
let carregarUtilizadoresView = () => {
    ...
    <td>${u.password}</td>
        ...
        <td>
        <button class="btn btn-warning btn-sm me-2" onclick="abrirformulárioEdit(${u.id}, '${u.nome}', '${u.email}','${u.password}', '${u.tipo}')">✏️</button>
...
let editarUtilizadorView = () => {
    ...
    editarUtilizador(id, nome, email, password, tipo);
...
let abrirformulárioEdit = (id, nome, email, password, tipo) => {
    document.getElementById('formularioUtilizador').style.display = 'none';
